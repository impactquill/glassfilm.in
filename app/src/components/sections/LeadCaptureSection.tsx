...
export default function LeadCaptureSection() {
  const { handleBookingSubmit, formSubmitted } = useBooking()
    ...
    <section id="contact" className="section-padding gradient-teal">
      <div className="container-custom">
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div className="bg-white p-8 rounded-2xl shadow-xl min-h-[400px] flex flex-col justify-center">
            {formSubmitted ? (
              <div className="text-center py-8 animate-in fade-in zoom-in duration-500">
                <div className="w-16 h-16 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                  <Check className="w-8 h-8 text-green-600" />
                </div>
                <h3 className="font-playfair text-2xl font-semibold text-charcoal mb-2">Thank You!</h3>
                <p className="text-slate text-lg">Thank you for the details, our team will reach out shortly</p>
              </div>
            ) : (
              <form onSubmit={handleBookingSubmit} className="space-y-5">
                ...
                <div>
                  <Label htmlFor="message" className="mb-2 block">Additional Details / Message</Label>
                  <textarea
                    id="message"
                    name="message"
                    placeholder="Tell us about your requirements (e.g. dimensions, specific patterns...)"
                    rows={3}
                    className="w-full rounded-lg border border-slate-200 p-3 focus:outline-none focus:ring-2 focus:ring-teal/30 focus:border-teal/50 transition-all text-sm"
                  />
                </div>
                <div>
